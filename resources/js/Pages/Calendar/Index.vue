    ...
    <AuthenticatedLayout>
        ...
        <div class="py-2">
            <div class="mx-auto max-w-7xl">
                ...
                <!-- Calendar Grid -->
                <div class="bg-white dark:bg-brand-dark-surface rounded-xl shadow-ambient dark:shadow-none border border-brand-primary/10 dark:border-brand-dark-border overflow-hidden relative">
                    <div class="grid grid-cols-7 border-b border-brand-primary/20 dark:border-brand-dark-border bg-white dark:bg-brand-dark-surface">
                        <div v-for="day in ['MON', 'TUE', 'WED', 'THUR', 'FRI', 'SAT', 'SUN']" :key="day" class="py-4 px-4 text-xs font-semibold text-brand-muted dark:text-brand-cream/70 uppercase tracking-wider border-r border-brand-primary/10 dark:border-brand-dark-border last:border-r-0">
                            {{ day }}
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-7 auto-rows-[160px]">
                        <div v-for="(dayObj, index) in calendarDays" :key="index" 
                             class="p-3 border-b border-r border-brand-primary/10 dark:border-brand-dark-border flex flex-col transition-colors relative group"
                             :class="{ 'bg-brand-primary/5': !dayObj, 'hover:bg-brand-cream dark:bg-brand-dark-base/20 cursor-pointer': dayObj && getBookingsForDate(dayObj.dateString).length === 0, 'border-r-0': (index + 1) % 7 === 0 }"
                             @click="dayObj && toggleBlockDate(dayObj.dateString)"
                             >
                            
                            <template v-if="dayObj">
                                <span class="text-sm font-semibold text-brand-primary dark:text-brand-cream mb-2 px-1">{{ dayObj.day }}</span>
                                ...
                                <div class="mt-auto space-y-1">
                                    <div v-if="getBookingsForDate(dayObj.dateString).length > 0" class="flex flex-col gap-1.5">
                                        <button v-for="booking in getBookingsForDate(dayObj.dateString)" :key="booking.id"
                                            ...
                                            class="w-full text-center px-2 py-1.5 bg-brand-primary/10 hover:bg-brand-primary/20 text-brand-primary dark:text-brand-cream text-xs font-bold rounded-lg transition-colors truncate"
                                        >
                                            View Booking
                                        </button>
                                        <span class="w-full text-center px-2 py-1 bg-green-200 dark:bg-green-500/10 text-green-700 dark:text-green-400 text-xs font-bold rounded-lg block border border-transparent dark:border-green-500/20">
                                            Booked
                                        </span>
                                    </div>
                                    <div v-else-if="isDateBlocked(dayObj.dateString)">
                                        <span class="w-full text-center px-2 py-1 bg-red-200 dark:bg-red-500/10 text-red-700 dark:text-red-400 text-xs font-bold rounded-lg block border border-transparent dark:border-red-500/20">
                                            Marked as Unavailable
                                        </span>
                                    </div>
                                    <div v-else>
                                        <span class="w-full text-center px-2 py-1 bg-cyan-100 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 text-xs font-bold rounded-lg block border border-transparent dark:border-cyan-500/20">
                                            Available
                                        </span>
                                    </div>
                                </div>
